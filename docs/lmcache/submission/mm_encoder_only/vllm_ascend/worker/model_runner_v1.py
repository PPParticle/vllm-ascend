...
class NPUModelRunner(GPUModelRunner):
    ...
    def _dummy_run(
        ...
        # [CCF Adaptation] Skip LM dummy run in encoder-only mode
        # (aligned with upstream GPUModelRunner behavior)
    ...
    def _dummy_sampler_run(
        ...
        # [CCF Adaptation] Skip sampler dummy run in encoder-only mode
        # (aligned with upstream GPUModelRunner behavior)
        mm_config = self.vllm_config.model_config.multimodal_config
        if mm_config and mm_config.mm_encoder_only:
            return torch.tensor([])
