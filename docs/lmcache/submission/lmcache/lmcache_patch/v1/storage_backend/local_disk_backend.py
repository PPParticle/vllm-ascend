...
class LocalDiskBackend(StorageBackendInterface):
    ...
    async def batched_get_non_blocking(
        ...
        for key in keys:
            ...
            # [CCF Adaptation] Use staging allocator instead of regular allocate
            # to prevent SSD reads from competing with hot_cache for CPU memory.
            ...
            memory_obj = self.local_cpu_backend.allocate_staging(
                shape,
                dtype,
                fmt,
                busy_loop=False,
    ...
    def load_bytes_from_disk(
        ...
        # [CCF Adaptation] Use staging allocator for disk read staging memory
        memory_obj = self.local_cpu_backend.allocate_staging(shape, dtype, fmt)
        if memory_obj is None:
            logger.error(
                "Staging allocation failed for disk load of %s. "
