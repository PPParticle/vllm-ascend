...
MODEL_PATH = os.environ.get("MODEL_PATH", "/data/models/Qwen2.5-14B-Instruct")
MODEL_NAME = MODEL_PATH  # Must match server's served_model_name
SERVER_PORT = 8100
SERVER_URL = f"http://localhost:{SERVER_PORT}"

CPU_CACHE_GB = float(os.environ.get("CPU_CACHE_GB", 8))
SSD_CACHE_GB = float(os.environ.get("SSD_CACHE_GB", 20))
SSD_PATH = os.environ.get("SSD_PATH", "/data/lmcache-disk/")
SERVER_LOG = os.environ.get("SERVER_LOG", "/tmp/benchmark_server.log")

MAX_OUTPUT_TOKENS = 10
MAX_NUM_SEQS = 8

# KV bytes per token: 48 layers × 2(K+V) × 8 KV heads × 128 dim × 2 bytes(fp16)
BYTES_PER_TOKEN = 48 * 2 * 8 * 128 * 2  # 196,608 bytes = 192 KB (Qwen2.5-14B)

CONTEXT_LENGTHS = [8192, 16384, 32768]
WORKING_SET_SIZES = [8, 16, 32]

# 14B matrix: KV/token is 3.43x the 7B value, so ws32@16k (103G) and
# ws32@32k are infeasible on this host (57G RAM + 18G /data).
TEST_MATRIX = [
    (ctx, ws)
    for ctx in CONTEXT_LENGTHS
    for ws in WORKING_SET_SIZES
    if not (ctx == 32768 and ws == 32) and not (ctx == 16384 and ws == 32)
]

SCBENCH_SUBSET = "scbench_qa_eng"  # 69 samples, 5+ multi-turns, context > 70k tokens
RESULTS_DIR = os.environ.get("RESULTS_DIR", "/home/fcqiao/v018-tests/perf_results/scbench")
...
def get_server_env(baseline=False):
    ...
    base_env = {
        ...
        # builtin-hash fallback needs a fixed seed, else store/lookup keys mismatch
        "PYTHONHASHSEED": "0",
    }
    if not baseline:
        base_env.update({
...
def load_scbench_data(subset, context_len, working_set_size, tokenizer, offset=0):
    ...
    ds = load_dataset(
        "microsoft/SCBench", subset, split="test", cache_dir="/data/hf-cache/hub"
    )
